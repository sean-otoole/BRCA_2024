...
Additionally, the cosine similarity was calculated by taking the averaged expression levels for the atypical memory cells from the Ma et al. paper and then calculating (which were derived the corrected normalized expression values from the SCTransform function after correcting for patient origin, mitochondrial percentage, and feature as well as total count per cell. Additionally, the expression vector only included highly variable genes within the Ma et. al data set. This averaged vector of highly variable genes was used for calculating the cosine similarity for each spot in the visium data set. The visium expression values were also corrected after adjusting for sample origin, mitochondrial percent, total features, and counts.
When examining the cosine similarity scores for the double-negative B-cell population derived from the Ma et al. dataset (shown below), there were clear differences between samples. However, this metric disagrees with the other two metrics reported. It will likely require some further validation. Additionally, the lack of bimodality indicates that this metric is not specific enough, potentially due to poor gene feature selection.
