...
XXX I can simply the categories to make things more clear ###
...
XXX explanation of gene sigs XXX

Here is one interesting gene sig:
...
XXX as well as another gene sig XXX
...
# Moran's I analysis LSM4

XXX include differential expression table XXX

XXX as well as another gene sig XXX

```{r grab_LSM4, echo=FALSE, fig.show='hide',results ='hide'}
# Specify the path to the R script
script_path <- paste0(getwd(), '/spatial_metric_description.r')
# Run the R script
source(script_path)
...
suppressWarnings(suppressMessages({
       spatial_metrics(current_metric, gene = TRUE)  # Call the function defined in the script
   }))

```

```{r display_LSM4, echo=FALSE}
current_metric <- "LSM4"
plot_path <- paste0(getwd(), '/figures/', current_metric, '_summary.png')
knitr::include_graphics(plot_path)
```
