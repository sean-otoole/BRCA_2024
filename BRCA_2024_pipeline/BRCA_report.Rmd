...
output: 
  prettydoc::html_pretty:
    ...
    toc: true
params:
  run_python_script: TRUE  # determines if the python pipeline will ran, or if the script relies on already stored objects
...
Below, several quality metrics are displayed, which include the number of unique features (left), mitochondrial features (center), and the number of unique molecular identifiers mapping to the transcriptome for each of the datasets (right).
...
## Examination of additional potentially relevant gene signatures

Although not directly relevant to the investigation of the double-negative B-cell subtype, several other gene signatures showed reasonably large differences between conditions. The first relates to natural killer cells. A summary of the results for this gene signature is displayed below in a manner consistent with the previous figure. While the sample-averaged values compared between conditions are not significant, the overall difference between distributions is most likely significant. However, I did not investigate this further, as it was not the focus of this analysis. I simply wanted to highlight it as a potential new area of investigation.
...
knitr::kable(table_stats, caption = "Table: DNB Violin Cell Statistics with Scientific Notation")
