...
I re-examined this gene signature, but instead of using global normalization of the expression matrix to calculate gene signature scores, I used the corrected expression matrix generated by Seurat's SCTransform. SCTransform employs a generalized linear model (GLM) based on a negative binomial distribution to model and correct for technical variation, such as sequencing depth. It removes this technical variation, stabilizes variance, and returns Pearson residuals as normalized expression values that better represent biological variation. Below is the same score calculated based on the corrected expression values.

```{r grab_double_neg_sct, echo=FALSE, fig.show='hide',results ='hide'}
# Specify the path to the R script
script_path <- paste0(getwd(), '/spatial_metric_description.r')
# Run the R script
source(script_path)
current_metric <- "DN2_Bcells_SCT"
...
Additionally the cosine similiarity was calcualted by taking the averaged expression levels for the aytpical memory cells from the Ma et al. paper and then calculating (which were derived the corrected normalized expression values from the SCTransform function after correcting for patient origin, mitochonrial percentage, and feature as well as total count per cell. Additionally, the expression vector only included highly variable genes within the Ma et. al data set. This averargaged vector of highly variable genes was uused for calcuting the cosine simialrity for each spot in the cisium data set. The visium expression values were also the corrected expression values after adjusting for sample origin, mitochorndiral percent, total and features and counts.
There were clear differences between samples when examining the cosine similarity scores for the double-negative B-cell population derived from the Ma et al. dataset (shown below). However, this metric disagrees with the other two metrics reported. It will likely require some further validation. Additionally, the lack of bimodality indidcates that this mmetric is not specific enough. Potentially due to poor gene feature selection.
